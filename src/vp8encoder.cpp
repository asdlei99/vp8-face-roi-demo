...
bool VP8Encoder::SetRoiMap(size_t left, size_t top, size_t width, size_t height)
{
	vpx_roi_map_t roi;
	memset(&roi, 0, sizeof(roi));
  
	roi.rows = (config.g_h + 15) / 16;
	roi.cols = (config.g_w + 15) / 16;
	
	//Get ROI rectangle in rows/cols
	uint32_t l =  left / 16;
	uint32_t r = (left + width) / 16;
	uint32_t t =  top / 16;
	uint32_t b = (top + height) / 16;
	...
	for (unsigned int y = 0; y < roi.rows; ++y)
		for (unsigned int x = 0; x < roi.cols; ++x)
			roi.roi_map[y * roi.cols + x] = y>=t && y<=b && x>=l && x<=r;

	if (vpx_codec_control(&encoder, VP8E_SET_ROI_MAP, &roi))
		return Error("Failed to set active map");
...
bool VP8Encoder::UnsetRoiMap()
{
	...
	if (vpx_codec_control(&encoder, VP8E_SET_ROI_MAP, &roi))
		return Error("Failed to unset roi map");
}
